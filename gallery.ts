...
const configs: BloomConfig[] = [
    ...
    { label: "Decrypting", pattern: [0, 2, 4, 6, 8, 1, 3, 5, 7], srgb: [255, 107, 107], p3: [0.95, 0.44, 0.44], stagger: 165, hold: 420, fade: 380, offset: 4500 },
    { label: "Analyzing", pattern: [2, 5, 8, 1, 4, 7, 0, 3, 6], srgb: [207, 159, 255], p3: [0.78, 0.62, 0.95], stagger: 205, hold: 370, fade: 400, offset: 5000 },
    { label: "Indexing", pattern: [4, 0, 8, 2, 6, 1, 7, 3, 5], srgb: [255, 215, 0], p3: [0.95, 0.84, 0], stagger: 180, hold: 390, fade: 370, offset: 5500 },
];
...
function frame(now: number) {
    ...
    for (const { cells, config } of buttons) {
        const { pattern, srgb, p3, stagger, hold, fade, offset } = config;
        const visible = hold + fade;
        const cycle = pattern.length * stagger;
